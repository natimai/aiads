...
import { ArrowLeft, Plus, Trash2, Shield, Clock, CheckCircle2, AlertTriangle, Loader2, RefreshCw } from "lucide-react";
import { Link, useSearchParams } from "react-router-dom";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useAccounts } from "../contexts/AccountContext";
import { connectAccount, disconnectAccount, syncAccount, syncAllAccounts } from "../services/api";
...
export default function AccountSettings() {
  ...
  useEffect(() => {
    ...
    if (success === "true") {
      setToast({ type: "success", message: `Connected ${count || ""} ad account${count !== "1" ? "s" : ""} successfully!` });
  ...
  const syncOne = useMutation({
    ...
    onSuccess: (data) => {
      setToast({ type: "success", message: `Synced: ${data.campaigns} campaigns, ${data.insights} insights` });
      queryClient.invalidateQueries();
    },
    onError: (err: Error) => setToast({ type: "error", message: err.message }),
  });
    ...
    <div className="space-y-6">
      {toast && (
        <div className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-sm ${
          ...
          <button onClick={() => setToast(null)} className="text-xs opacity-60 hover:opacity-100">Dismiss</button>
        </div>
      )}

      <div className="flex items-center gap-4">
        <Link to="/settings" className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-slate-200">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div className="flex-1">
          <h2 className="text-xl font-bold text-white">Meta Ad Accounts</h2>
          <p className="text-sm text-slate-400">Connect and manage your Meta advertising accounts</p>
        </div>
      ...
      {accounts.length > 0 ? (
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-4 text-xs text-slate-400">
                    ...
                    {account.businessName && <span>Business: {account.businessName}</span>}
                  </div>
                  {account.tokenExpiry && (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-slate-800 bg-navy-900 py-16">
          ...
          <p className="mt-1 text-xs text-slate-500">Click "Connect Account" to link your Meta Ad Account</p>
        </div>
      )}
    </div>
  );
