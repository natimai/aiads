...
export function SpendDistribution({ campaigns, currency = "USD", loading }: SpendDistributionProps) {
  if (loading) {
    return (
      <div className="panel p-5">
        <div className="mb-4 h-4 w-36 rounded bg-[var(--line)] skeleton" />
        <div className="mx-auto h-44 w-44 rounded-full bg-[var(--line)] skeleton" />
      </div>
    );
  }
  ...
  return (
    <div className="panel p-5">
      <h3 className="mb-4 text-sm font-semibold text-[var(--text-primary)]">התפלגות הוצאה</h3>
            ...
            {data.map((d, i) => (
              <div key={d.name} className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="truncate text-[12px] text-[var(--text-secondary)]">{d.name}</span>
                </div>
                <div className="shrink-0 text-right">
                  <span className="font-mono text-[12px] text-[var(--text-primary)]">{formatCurrency(d.value, currency)}</span>
